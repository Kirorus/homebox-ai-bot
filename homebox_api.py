...
class HomeboxAPI:
            ...
            async with session.post(
                ...
                if photo_path and item_id:
                    ...
                    if not uploaded:
                        ...
                        logger.info(f"Trying alternative upload method for item {item_id}")
                        uploaded = await self.upload_photo_alternative(item_id, photo_path)
                        if not uploaded:
                            # Пробуем base64 метод
                            logger.info(f"Trying base64 upload method for item {item_id}")
                            uploaded = await self.upload_photo_base64(item_id, photo_path)
                            if not uploaded:
                                # Пробуем curl-style метод
                                logger.info(f"Trying curl-style upload method for item {item_id}")
                                uploaded = await self.upload_photo_curl_style(item_id, photo_path)
                                if not uploaded:
                                    # Пробуем raw HTTP метод
                                    logger.info(f"Trying raw HTTP upload method for item {item_id}")
                                    uploaded = await self.upload_photo_raw_http(item_id, photo_path)
                                    if not uploaded:
                                        # Вернём флаг/сообщение, чтобы бот мог показать пользователю
                                        item['photo_upload'] = 'failed'
                                        logger.warning(f"All 5 upload methods failed for item {item_id}")
                                    else:
                                        logger.info(f"Raw HTTP upload succeeded for item {item_id}")
                                else:
                                    logger.info(f"Curl-style upload succeeded for item {item_id}")
                            else:
                                logger.info(f"Base64 upload succeeded for item {item_id}")
                        else:
                            logger.info(f"Alternative upload succeeded for item {item_id}")
                    else:
                        logger.info(f"Standard upload succeeded for item {item_id}")
    ...
    async def upload_photo_raw_http(self, item_id: str, photo_path: str) -> bool:
